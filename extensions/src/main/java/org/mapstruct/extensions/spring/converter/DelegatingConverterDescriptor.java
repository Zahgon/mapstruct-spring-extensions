...
public class DelegatingConverterDescriptor {
    ...
    public DelegatingConverterDescriptor(final ExecutableElement annotatedMethod, final ProcessingEnvironment processingEnv) {
        ...
        if (parameterList.size() != 1 || TypeKind.VOID.equals(returnType.getKind())) {
            final var errorMessage = "Can only generate a delegating converter for methods with exactly one parameter and non-void return type.";
            processingEnv.getMessager().printMessage(Diagnostic.Kind.ERROR, errorMessage, annotatedMethod);
            throw new IllegalArgumentException(errorMessage);
        }
        fromToMapping = new FromToMapping().source(TypeName.get(parameterList.iterator().next().asType())).target(TypeName.get(returnType));
        originalMapperClassName = ClassName.get((TypeElement) annotatedMethod.getEnclosingElement());
        converterClassName = ClassName.get(originalMapperClassName.packageName(), String.format("%sTo%sConverter", collectionOfNameIfApplicable(processingEnv, fromToMapping.getSource()), collectionOfNameIfApplicable(processingEnv, fromToMapping.getTarget())));
